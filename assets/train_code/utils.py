...
class conv_residual_block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=None, down_sample=None, groups=1, last_activation=True, norm_layers=False):
        ...
        # if down_sample:
            # self.shortcut = nn.Conv2d(in_channels,  out_channels,   kernel_size=1,  stride=2,   padding=0, groups=groups)
        # else:
        self.shortcut = nn.Conv2d(in_channels,  out_channels,   kernel_size=1,  stride=1,   padding=0, groups=groups)

    def forward(self, x):
        if self.down_sample == 'avg':
            out = self.avg_pool(x)
        elif self.down_sample == 'max':
            out = self.max_pool(x)
        elif not self.down_sample:
            out = x
        else:
            raise ValueError('[model: conv_se_residual_block] unknown down_sample type!')
        out = self.conv1(out)
        if self.norm_layers:
            out = self.bn1(out)
        out = self.relu(out)
...
class fc_residual_block(nn.Module):
    ...
    def __init__(self, input_units, output_units=None, last_activation=True, norm_layers=False):
        ...
        if output_units == input_units:
            middle_units = output_units // 2
        else:
            middle_units = output_units
