...
from scripts.render_buffer import render
        ...
        while True:
            ...
            if self.config.log_file:
                ...
                if ( 
                    ...
                    out_dir = os.path.join(self.config.save_root, self.start_time, f'epoch-{epoch+1}')
    ...
    def render_jobs(self, out_dir):
        ...
        with tqdm.tqdm(self.config.train_materials) as pbar:
            for i, m in enumerate(pbar):
                ...
                render(*args, './buffer/buffer_plane_collocated.exr', os.path.join(out_dir, f'plane_train_col_{m}.exr'), self.config.device, output_brdf_value=False)
                render(*args, './buffer/buffer_plane_point_persp.exr', os.path.join(out_dir, f'plane_train_point_persp_{m}.exr'), self.config.device, output_brdf_value=False)
                render(*args, './buffer/buffer_plane_parallax.exr', os.path.join(out_dir, f'plane_train_parallax_{m}.exr'), self.config.device, output_brdf_value=False)
                # render(*args, './buffer/buffer_sphere_dir_orth.exr', os.path.join(out_dir, f'sphere_train_BRDFval_{m}.exr'), self.config.device, output_brdf_value=True)
