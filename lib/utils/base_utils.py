...
def coordinateSystem(axis_a):
    ...
    axis_c = torch.stack([axis_c0, axis_c1], dim=0).gather(dim=0,
                                                           index=nxy_argmax.expand(*nxy_argmax.shape[:-1], 3).unsqueeze(
                                                               0)).squeeze(0)
...
def xy_to_xyz(vec):
    return torch.cat(
        [vec[..., 0:1], vec[..., 1:2], torch.sqrt(torch.clamp(1 - vec[..., 0:1] ** 2 - vec[..., 1:2] ** 2, 0, 1))],
        dim=-1)
...
def rotate_along_axis(vec, axis, angle):
    ...
    out = vec * cos_angle
    out += axis * (axis[..., 0:1] * vec[..., 0:1] + axis[..., 1:2] * vec[..., 1:2] + axis[..., 2:3] * vec[..., 2:3]) * (
                1 - cos_angle)
