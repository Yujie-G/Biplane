...
class NormalMap(nn.Module):
    ...
    def __init__(self, config: RepConfig):
        ...
        self.normal = nn.Parameter(torch.tensor([0., 0., 1.], dtype=torch.float32, device=self.config.device, requires_grad=True).reshape(1, 1, 1, 3).repeat(self.M, self.U, self.V, 1))
        
    def get_param_count(self):
        return (f'{self.M} x {self.U} x {self.V} x {3} = {self.M * self.U * self.V * 3:,d} ')
    
    def vis_feature(self, out_dir, epoch):
        import os
        self.output_normalmap(out_path=os.path.join(out_dir, f'epoch-{epoch}_feature_vis', 'normalmap.exr'))    

    def output_normalmap(self, out_path='normal.exr'):
        img = normalize(self.normal).detach().cpu().numpy().reshape(self.M, self.U, self.V, 3)
        img[..., 0:2] = (img[..., 0:2] + 1) * 0.5
        from lib.utils.feature_vis import write_blocks
        write_blocks(img, out_path)
    ...
    def nearest(self, x, m, i, j, I, J):
        i1, j1 = map(lambda x: torch.round(x).long(), (i, j))
        i1[i1 == I] = I - 1
        j1[j1 == J] = J - 1
        return x[m, i1, j1, :]
    
    def forward(self, m, h, u, v):
        ind_u, ind_v = self.get_uv_indices(u, v)
        frame = Frame(self.nearest(normalize(self.normal), m, ind_u, ind_v, self.U, self.V))
        return frame.to_local(xy_to_xyz(h))
...
class Offset(nn.Module):
    ...
    def __init__(self, config: RepConfig, init='zeros'):
        ...
        self.L = self.config.latent_size[2]
        self.output_channels = 1
        self.offset_texture = nn.Parameter(getattr(torch, init)(self.M, self.U,  self.V,  self.L, requires_grad=True, device=config.device, dtype=torch.float32))
        self.init_offset_network()
        
    def init_offset_network(self):
        channels = 32
        self.offset_network = nn.Sequential(
            fcrelu(self.L + 2, channels),
            fcrelu(channels, channels),
            nn.Linear(channels, self.output_channels),
            # nn.Sigmoid(),
        )
        
    def get_param_count(self):
        return (f'{self.M} x {self.U} x {self.V} x {self.L} = {self.M * self.U * self.V * self.L:,d} ')

    def output_depth(self, wi=[0.5774, 0.5774, 0.5774], out_path='depth.exr'):
        from lib.utils import exr
        with torch.no_grad():
            wi = torch.tensor(wi, dtype=torch.float32, device=self.offset_texture.device).reshape(1, 3).repeat(self.U * self.V, 1)
            if self.output_channels == 1:
                depth = self.offset_network(torch.cat([self.offset_texture[0].reshape(-1, self.L), wi[..., :2]], -1)).cpu().numpy().reshape(self.config.btf_size[0], self.config.btf_size[1], -1).transpose(1, 0, 2)
                exr.write(depth[..., 0], out_path)
            else:
                offset = self.offset_network(torch.cat([self.offset_texture[0].reshape(-1, self.L), wi[..., :2]], -1)).cpu().numpy().reshape(self.config.btf_size[0], self.config.btf_size[1], -1).transpose(1, 0, 2)
                exr.write(np.concatenate([offset, np.ones_like(offset[..., :1])], -1), out_path)

    def vis_feature(self, out_dir, epoch):
        from lib.utils.feature_vis import write_blocks
        import os
        write_blocks(self.offset_texture.detach().cpu().numpy(), os.path.join(out_dir, f'epoch-{epoch}_feature_vis', 'offset_texture.exr'))
        self.output_depth(wi=[0, 0.707, 0.707],  out_path=os.path.join(out_dir, f'epoch-{epoch}_feature_vis', 'depth_down.exr'))
        self.output_depth(wi=[0.707, 0, 0.707],  out_path=os.path.join(out_dir, f'epoch-{epoch}_feature_vis', 'depth_right.exr'))
        self.output_depth(wi=[0, -0.707, 0.707], out_path=os.path.join(out_dir, f'epoch-{epoch}_feature_vis', 'depth_up.exr'))
        self.output_depth(wi=[-0.707, 0, 0.707], out_path=os.path.join(out_dir, f'epoch-{epoch}_feature_vis', 'depth_left.exr'))
    ...
    def forward(self, m, u, v, wi, radius=0):
        ## h: [bs, 2], u: [bs, 1], v: [bs, 1]
        ind_u, ind_v = self.get_uv_indices(u, v)
        latent = self.bilinear_interp(self.blur(self.offset_texture, radius), m, ind_u,  ind_v,  self.U , self.V )
        if self.output_channels == 1:
            depth = self.offset_network(torch.cat([latent.reshape(-1, self.L), wi.reshape(-1, 3)[..., :2]], dim=-1)).reshape(*u.shape, 1)
            offset = depth / torch.clamp(wi[..., 2:], 1e-6, 6) * -wi[..., :2]
        else:
            offset = self.offset_network(torch.cat([latent.reshape(-1, self.L), wi.reshape(-1, 3)[..., :2]], dim=-1)).reshape(*u.shape, 2)
        return (offset[..., 0] + u) % 1.0, (offset[..., 1] + v) % 1.0, offset
